...
void DepthCameraPlugin::Load(sensors::SensorPtr _sensor,
	...
	this->newDepthFrameConnection = this->depthCamera->ConnectNewDepthFrame(
			boost::bind(&DepthCameraPlugin::OnNewDepthFrame, this, _1, _2, _3,
					_4, _5));

	this->newRGBPointCloudConnection =
			this->depthCamera->ConnectNewRGBPointCloud(
					boost::bind(&DepthCameraPlugin::OnNewRGBPointCloud, this,
							_1, _2, _3, _4, _5));

	this->newImageFrameConnection = this->depthCamera->ConnectNewImageFrame(
			boost::bind(&DepthCameraPlugin::OnNewImageFrame, this, _1, _2, _3,
					_4, _5));
...
void DepthCameraPlugin::OnNewDepthFrame(const float *_image,
	...
	std::string *fileName;
	fileName->append("/tmp/DepthCam/me.jpg");
	unsigned char *imageParam;
	imageParam = (unsigned char*) _image;
	...
	printf("W[%d] H[%d] MidPoint[%d] Dist[%f] Min[%f] Max[%f]\n", width, height,
			index, _image[index], min, max);

//	rendering::Camera::SaveFrame(_image, this->width, this->height, this->depth,
//			this->format, "/tmp/depthCamera/me.jpg");

//	if (rendering::Camera::SaveFrame(imageParam, this->width, this->height,
//			this->depth, this->format, "/tmp/depthCamera/me.jpg")) {
//		std::cout << "FRAME SAVED!" << std::endl;
//	}

	rendering::Camera::SaveFrame(imageParam, this->width, this->height,
			this->depth, this->format, "/tmp/depthCamera/me.jpg");
...
void GazeboRosOpenniKinect::OnUpdate() {
	std::cout << "UPDATE FROM OPENNI KINECT!!" << std::endl;
}
