...
void save_proba(const pcl::PointCloud<PointT>::Ptr cloud){
    ...
    std::cout<<"Start estimating normals, cloud size:"<< cloud->size()<<std::endl;
    ...
    pcl::search::KdTree<PointTNormal>::Ptr tree2 (new pcl::search::KdTree<PointTNormal>);
...
//       gp3.setNormalConsistency(false);
    ...
    pcl::io::savePolygonFile("/home/furdek/catkin_ws/save_proba_mesh.obj", triangles);
...
//    pcl::io::savePolygonFileVTK("/home/furdek/catkin_ws/save_proba.vtk", triangles);
    pcl::io::savePolygonFileSTL("/home/furdek/catkin_ws/save_proba.stl", triangles);
...
void save_pcl_VTK(const pcl::PointCloud<PointT>::Ptr cloud, const std::string str){
       ...
       pcl::PointCloud<PointTNormal>::Ptr mls_points (new pcl::PointCloud<PointTNormal>);
...
//       poly_data->Update();
    ...
    std::cout << "MESH POINTS READ FROM VTK FILE: " << my_mesh.cloud.data.size() << std::endl;
...
void save_VTK_to_binary_STL(const std::string path_to_read_from,
    ...
    vtkSmartPointer<vtkInformation> vtk_info = vtkSmartPointer<vtkInformation>::New();
    vtkSmartPointer<vtkPolyData> poly_data = vtkSmartPointer<vtkPolyData>::New();
    vtkSmartPointer<vtkDataObject> data_object = vtkSmartPointer<vtkDataObject>::New();
...
//    triangles->Update();
    ...
    // ########################### READS VTK FILE WELL ########################## //
    vtkSmartPointer<vtkGenericDataObjectReader> vtk_reader = vtkSmartPointer<vtkGenericDataObjectReader>::New(); //vtkSTLReader
    vtk_reader->SetFileName(path_to_read_from.c_str());
    vtk_reader->Update();
...
std::vector<pcl::PointCloud<PointT> > separate_2d_clusters(const pcl::PointCloud<PointT>::Ptr cloud){
...
void ransac_line_fitting(std::vector<pcl::PointCloud<PointT> > cluster_vect) {
    ...
    *cloud = cluster_vect[a];
	...
	if (ransac_line_inliers.size()) {
        std::cout << "Writing LINES Cloud to:" << ss.str()
				<< std::endl;
		pcl::PCDWriter writer;
...
//	std::vector<pcl::PointIndices> euclidean_label_indices;
	...
	if (regions.size() > 0) {
		...
		for (size_t i = 0; i < label_indices.size(); i++) {
			if (label_indices[i].indices.size() > 30000) {
				plane_labels[i] = true;
			}
		}
		...
		for (size_t i = 0; i < euclidean_label_indices.size(); i++) {
			...
			if (euclidean_label_indices[i].indices.size() > 1000) {
                ...
                std::cerr << "Writing CLUSTER PCD's to: " << clusterPCDPath << std::endl;
...
std::vector<float> pointsIn3DRegion(
    ...
    pcl::PointCloud<PointT>::Ptr cluster_cloud_ptr (new pcl::PointCloud<PointT>());
	...
	for (size_t i = 0; i < clusters.size(); i++) {
		...
		if (contor == cluster_cloud.size()) {
            pcl::io::savePCDFile("/home/furdek/catkin_ws/cloud_4_stefan.pcd",cluster_cloud);
            ...
            ss << VTKPath << i << ".vtk";
            pcl::io::loadPCDFile("/home/furdek/catkin_ws/cloud_4_stefan.pcd", *proba_cloud);
...
void final_filter() {
    ...
    std::cout
            << "Writing PCD after final_filter to /home/furdek/final.pcd"
            << std::endl;
