...
GazeboRosOpenniKinect::GazeboRosOpenniKinect() {
	...
	this->CameraInfoTopicName = "/kinect_head/depth_registered/camera_info";
...
void GazeboRosOpenniKinect::Load(sensors::SensorPtr _parent,
	...
	std::cout << "MY OPENNI KINECT LOADER" << std::endl;
	...
	this->SubscribeToCameraInfo();

}

void GazeboRosOpenniKinect::Advertise() {
	ros::AdvertiseOptions point_cloud_ao = ros::AdvertiseOptions::create<
			sensor_msgs::PointCloud2>(this->point_cloud_topic_name_, 1,
	...
	ros::AdvertiseOptions depth_image_ao = ros::AdvertiseOptions::create<
			sensor_msgs::Image>(this->depth_image_topic_name_, 1,
	...
	ros::AdvertiseOptions depth_image_camera_info_ao =
			ros::AdvertiseOptions::create<sensor_msgs::CameraInfo>(
					...
					boost::bind(&GazeboRosOpenniKinect::DepthInfoDisconnect,
							this), ros::VoidPtr(), &this->camera_queue_);
	this->depth_image_camera_info_pub_ = this->rosnode_->advertise(
			depth_image_camera_info_ao);
}

void GazeboRosOpenniKinect::OnUpdate() {
//	std::cout << this->camera_->GetHFOV() << std::endl;
//	std::cout << "WIDTH" << this->width << std::endl;
}

void GazeboRosOpenniKinect::CameraTopicCallback(
		const sensor_msgs::CameraInfo::ConstPtr &message) {

	std::cout << "Callback from CameraInfo" << std::endl;
	std::cout << "VALUE OF Fx: " << message->P.elems[0] << std::endl;
	this->camera_->SetHFOV(this->CalculateFovX(message->P.elems[0]));
	this->CameraInfoSubscriber.shutdown();
}

math::Angle GazeboRosOpenniKinect::CalculateFovX(float P_value) {

	math::Angle myAngle;
	double fovx;
	fovx = 2 * (atan((double) (this->width / (2.0 * P_value))));
	myAngle.SetFromRadian(fovx);
	return myAngle;

}

void GazeboRosOpenniKinect::SubscribeToCameraInfo() {
	this->CameraInfoSubscriber = this->CameraNode.subscribe(
			this->CameraInfoTopicName, 1000,
			&GazeboRosOpenniKinect::CameraTopicCallback, this);
}
...
void GazeboRosOpenniKinect::FillPointdCloud(const float *_src) {
	...
	this->point_cloud_msg_.row_step = this->point_cloud_msg_.point_step
			* this->width;

	///copy from depth to point cloud message
	FillPointCloudHelper(this->point_cloud_msg_, this->height, this->width,
...
bool GazeboRosOpenniKinect::FillDepthImageHelper(sensor_msgs::Image& image_msg,
		uint32_t rows_arg, uint32_t cols_arg, uint32_t step_arg,
		void* data_arg) {
...
void GazeboRosOpenniKinect::PublishCameraInfo() {
	...
	if (this->depth_info_connect_count_ > 0) {
		...
		if (cur_time - this->last_depth_image_camera_info_update_time_
				>= this->update_period_) {
